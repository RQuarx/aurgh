...
{
    void
    flowbox_remove_children( Gtk::FlowBox *fb )
    {
        fb->foreach([fb](Gtk::Widget &w){
            fb->remove(w);
        });
    }
                ...
                CriteriaType     type )
{
    ...
    if (type == SEARCH_ENTRY) {
        ...
        flowbox_remove_children(m_content);
        ...
        std::ranges::for_each(get_pkgs_info(sorted),
        [this]( const Json::Value &pkg )
        {
            m_cards.emplace_back(m_logger, pkg);

            if (!m_cards.back().is_valid()) {
                m_logger->log<ERROR>("Invalid card for {}.",
                                      pkg["Name"].asString());
                return;
            }

            m_content->add(*m_cards.back().get_widget());
        });
    } else {
                   ...
                   const std::string &search_by ) -> Json::Value
{
    std::string url { std::format("{}/search/{}?by={}", AUR_URL,
                                                        pkg, search_by) };
    m_logger->log<INFO>("Searching for {} using url {}", pkg, url);
