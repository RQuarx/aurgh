...
public:
    ...
    Logger(const std::string &p_log_level, const std::string &p_log_file = "");


    template <LogLevel T_Level, typename... T_Args>
    void
    log(StringSource p_fmt, T_Args &&...p_args)
    {
        ...
        if (func.contains(' ')) func = func.substr(func.find_first_of(' ') + 1);

        std::string label { std::format(
            "{} {} at \033[1m{}\033[0m( \033[1;30m{}:{}\033[0;0m )", get_time(),
            m_LABELS.at(T_Level).first, func, file, line) };

        if (m_log_file.is_open())
        {
            std::string file_label { std::format(
                "{} at {}( {}:{} )", get_time(), m_LABELS.at(T_Level).second,
                func, file, line) };
